	...
	def initialize_data(project_name, is_lte=False, overwrite_plants=False):
		...
		if decision_table.D_table_dtl.select().count() < 1:
			if not is_lte:
				...
				# note: drawdown_days=corps_med_res and flood_season=corps_med_res1
				valid_res_tables = ['corps_med_res1', 'corps_med_res', 'wetland', 'drawdown_days', 'flood_season']
				valid_file_types = ['lum.dtl', 'res_rel.dtl']
				query = dataset_dts.D_table_dtl.select().where((dataset_dts.D_table_dtl.file_name.in_(valid_file_types)) & ((dataset_dts.D_table_dtl.file_name != 'res_rel.dtl') | (dataset_dts.D_table_dtl.name.in_(valid_res_tables))))
			else:
			...
			for dt in query:
				d_id = decision_table.D_table_dtl.insert(name=dt.name, file_name=dt.file_name, description=dt.description).execute()
